...
// Agregamos Pencil y Trash2 para los botones de editar y borrar
...
export default function DashboardPro() {
  ...
  const [proyectoEditando, setProyectoEditando] = useState<string | null>(null) // Para saber si estamos editando
  ...
  const abrirModalEditar = (p: any, e: React.MouseEvent) => {
    e.preventDefault() // Evita que se abra el Link del proyecto al hacer clic en el botón
    ...
    <div className="min-h-screen bg-[#030712] text-slate-200 p-6 md:p-12 font-sans selection:bg-emerald-500/30">
      ...
      <main className="max-w-7xl mx-auto">
        ...
        {cargando ? (
          ...
          /* DISEÑO CUANDO NO HAY PROYECTOS (EMPTY STATE) */
          <div className="text-center py-24 glass-card rounded-3xl border-dashed border-2 border-white/10 flex flex-col items-center justify-center">
             ...
             <button onClick={abrirModalNuevo} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-3 rounded-xl font-bold transition-all shadow-[0_0_20px_rgba(52,211,153,0.3)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {proyectos.map((p) => (
              <Link href={`/obras/${p.id}`} key={p.id}>
                <div className="glass-card rounded-2xl p-6 hover:border-emerald-500/30 transition-all group cursor-pointer relative overflow-hidden h-full">
                  ...
                  <div className="flex justify-between items-center mb-6 relative">
                    <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border flex items-center gap-1.5 ${
                      p.estatus === 'En Pausa' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' :
                      p.estatus === 'Terminado' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                      'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                    }`}>
                      <Activity size={12} /> {p.estatus || 'Activo'}
                    </span>
                    
                    {/* BOTONES DE ACCIÓN */}
                    <div className="flex items-center gap-2">
                      <button onClick={(e) => abrirModalEditar(p, e)} className="p-2 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 rounded-lg transition-colors border border-transparent hover:border-blue-500/30">
                        <Pencil size={14} />
                      </button>
                      <button onClick={(e) => eliminarProyecto(p.id, e)} className="p-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg transition-colors border border-transparent hover:border-red-500/30">
                        <Trash2 size={14} />
                      </button>
      ...
      {/* MODAL CON ESTILO CRISTAL PARA NUEVO/EDITAR PROYECTO */}
      {mostrarModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-[#0B1221]/90 border border-white/10 rounded-3xl p-8 w-full max-w-md shadow-2xl relative glass-card">
            <button onClick={() => setMostrarModal(false)} className="absolute top-6 right-6 text-slate-400 hover:text-white transition-colors">
              <X size={24} />
            </button>
            ...
            <form onSubmit={guardarProyecto} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input required type="number" step="0.01" placeholder="0.00" value={nuevoPresupuesto} onChange={(e) => setNuevoPresupuesto(e.target.value)} className="w-full bg-black/30 border border-white/10 p-3.5 pl-8 rounded-xl text-emerald-400 font-black focus:border-emerald-500 outline-none transition-colors text-lg" />
                </div>
              </div>
              <button type="submit" disabled={guardando} className="w-full bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white font-black py-4 rounded-xl mt-6 transition-all shadow-[0_0_20px_rgba(52,211,153,0.3)]">
