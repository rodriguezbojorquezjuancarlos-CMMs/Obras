...
export default function Home() {
  ...
  // Modal States for New / Edit Project
  ...
  const abrirModalEditar = (proj: any, e: React.MouseEvent) => {
    ...
    setEstatus(proj.estatus || "ACTIVE")
    setMostrarModal(true)
  }
  ...
  const eliminarProyecto = async (id: string, e: React.MouseEvent) => {
    e.preventDefault()
    if (!window.confirm("Are you sure you want to delete this project and all its associated records?")) return
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="glass-card px-6 py-4 rounded-3xl mb-12 border border-white/10 flex flex-col md:flex-row justify-between items-center gap-4 bg-white/[0.02] backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black text-white tracking-tight">Kinetix <span className="text-emerald-400">Civil & Build</span></h1>
              <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest">Enterprise Construction Management</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <Link href="/cotizaciones" className="flex items-center gap-2 bg-blue-600/25 hover:bg-blue-600/40 border border-blue-500/30 text-blue-400 px-4 py-2.5 rounded-2xl font-bold transition-all text-sm cursor-pointer shadow-[0_0_15px_rgba(59,130,246,0.15)]">
              <FileText size={16} /> Estimates & Invoices
            </Link>
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-400 mt-1">Real-time supervision and financial control.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {proyectos.length === 0 ? (
            <div className="col-span-full py-16 text-center text-slate-500 glass-card rounded-3xl border border-white/5">
              No projects registered. Click "New Project" to get started.
            </div>
          ) : (
            proyectos.map(proj => (
              <Link href={`/obras/${proj.id}`} key={proj.id} className="group">
                <div className="glass-card p-6 rounded-3xl border border-white/10 hover:border-emerald-500/40 transition-all duration-300 relative overflow-hidden flex flex-col justify-between h-72 bg-gradient-to-b from-white/[0.03] to-transparent">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-4">
                      <select 
                        ...
                        className={`text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full border outline-none cursor-pointer ${
                          proj.estatus === 'TERMINADO' || proj.estatus === 'COMPLETED' ? 'bg-blue-500/10 text-blue-400 border-blue-500/30' :
                          proj.estatus === 'EN PAUSA' || proj.estatus === 'PAUSED' ? 'bg-amber-500/10 text-amber-400 border-amber-500/30' :
                          'bg-emerald-500/10 text-emerald-400 border-emerald-500/30'
                        }`}
                      >
                        <option value="ACTIVE" className="bg-[#0B1221] text-emerald-400">ACTIVE</option>
                        <option value="PAUSED" className="bg-[#0B1221] text-amber-400">PAUSED</option>
                        <option value="COMPLETED" className="bg-[#0B1221] text-blue-400">COMPLETED</option>
                      </select>

                      <div className="flex items-center gap-1.5 opacity-80 group-hover:opacity-100 transition-opacity">
                        <button onClick={(e) => abrirModalEditar(proj, e)} className="p-2 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 rounded-xl cursor-pointer" title="Edit project">
                          <Pencil size={14} />
                        </button>
                        <button onClick={(e) => eliminarProyecto(proj.id, e)} className="p-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-xl cursor-pointer" title="Delete project">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
                    ...
                    <p className="text-xs text-slate-400 mt-1 font-medium">{proj.ubicacion || 'No location specified'}</p>
                  </div>
      ...
      {mostrarModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-[#0B1221] border border-white/10 rounded-3xl p-8 w-full max-w-md shadow-2xl relative glass-card">
            ...
            <h2 className="text-2xl font-black text-white mb-6">{proyectoEditando ? "Edit Project" : "Create New Project"}</h2>
            
            <form onSubmit={guardarProyecto} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Project / Build Name</label>
                <input required type="text" placeholder="Ex. Tijuana House..." value={nombre} onChange={(e) => setNombre(e.target.value)} className="w-full bg-black/30 border border-white/10 p-3.5 rounded-xl text-slate-200 outline-none text-sm focus:border-emerald-500" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Location</label>
                <input type="text" placeholder="Ex. Tijuana, B.C." value={ubicacion} onChange={(e) => setUbicacion(e.target.value)} className="w-full bg-black/30 border border-white/10 p-3.5 rounded-xl text-slate-200 outline-none text-sm focus:border-emerald-500" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Client</label>
                  <input type="text" placeholder="Ex. John Doe" value={cliente} onChange={(e) => setCliente(e.target.value)} className="w-full bg-black/30 border border-white/10 p-3.5 rounded-xl text-slate-200 outline-none text-sm focus:border-emerald-500" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Budget (USD)</label>
              ...
              <button type="submit" disabled={guardando} className="w-full bg-emerald-500 hover:bg-emerald-400 text-black font-black py-4 rounded-xl mt-6 transition-all shadow-[0_0_20px_rgba(52,211,153,0.3)] cursor-pointer">
                {guardando ? "SAVING..." : proyectoEditando ? "UPDATE PROJECT" : "SAVE PROJECT"}
              </button>
            </form>
          </div>
        </div>
